...
const Index: FC = () => {
  ...
  const { data, error, isLoading } = useGetShowsById(playingData);
  ...
  return (
    <VStack>
      ...
      >
        {data?.map((show) => {
          const runtime =
            show.media_type === 'tv' ? show.episode_run_time[0] : show.runtime;
          return (
            show.poster_path && (
                ...
                <Link to={`/details/${show.id}?media_type=${show.media_type}`}>
                  <AspectRatio ratio={2 / 3}>
                    <Image
                      ...
                      alt={show.media_type === 'tv' ? show.name : show.title}
                    />
                  </AspectRatio>
                  <VStack mt={1}>
                    <Progress
                    ...
                    />
                    <Tooltip
                      label={show.media_type === 'tv' ? show.name : show.title}
                    >
