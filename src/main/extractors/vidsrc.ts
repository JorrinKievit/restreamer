...
export class VidSrcExtractor implements IExtractor {
  ...
  private getHashBasedOnIndex(hash: string, index: string) {
    ...
    for (let i = 0; i < hash.length; i += 2) {
      ...
      result += String.fromCharCode(
        parseInt(j, 16) ^ index.charCodeAt((i / 2) % index.length),
      );
    }
    return result;
  }
  ...
  async extractUrls(
    ...
    try {
      const url =
        type === "movie"
          ? `${this.embedUrl}movie?imdb=${imdbId}`
          : `${this.embedUrl}tv?imdb=${imdbId}&season=${season}&episode=${episode}`;
      ...
      const subtitleData = await axiosInstance.get(
        `${this.subtitleUrl}${imdbId}`,
        {
          headers: {
      ...
      const reducedSubtitles = subtitleData.data.reduce(
        (accumulator: any, subtitle: any) => {
      ...
      const finalSubtitles = Object.values(
        reducedSubtitles,
      ).flat() as Subtitle[];
