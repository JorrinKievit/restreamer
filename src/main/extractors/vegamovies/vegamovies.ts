...
export class VegaMoviesExtractor implements IExtractor {
  ...
  private async getTitleUrl(title: string) {
    const res = await axiosInstance.get(
      `${this.url}search/${encodeURIComponent(title)}`,
    );
    const $ = load(res.data);
  ...
  private async extractVCloud(url: string): Promise<Source> {
    ...
    if (!vCloudDownloadLink) {
      ...
      vCloudDownloadLink = Buffer.from(
        responseUrl.data.request.res.responseUrl.split("r=")[1],
        "base64",
      ).toString();
    }
    const redirectLinkData = await axiosInstance.get(vCloudDownloadLink, {
      headers: {
        cookie: vCloudPageCookies,
        "User-Agent":
          "Mozilla/5.0 (Linux; Android 10; SM-G973F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/94.0.4606.61 Mobile Safari/537.36",
      },
    });
    const $redirectLinkData = load(redirectLinkData.data);
    let finalDownloadLink = $redirectLinkData("a.btn-success.btn-lg.h6").attr(
      "href",
    );
    if (!finalDownloadLink) {
      finalDownloadLink = $redirectLinkData(".btn.btn-success.btn-lg.h6").attr(
        "href",
      );
    }
  ...
  public async extractUrls(
    ...
    try {
      const findMovieUrl = await this.getTitleUrl(
        this.mapWrongShowTitles(title),
      );
      ...
      if (type === "tv") {
        const seasonTitleContainer = moviePage$("h3:has(span) span")
          ...
          .last();
        const seasonButtonContainer = seasonTitleContainer
          .parentsUntil("h3")
          .parent()
          .next();
        const vCloudUrl = seasonButtonContainer
          ...
          .filter(
            (_, el) =>
              moviePage$(el).text().includes("V-Cloud") ||
              moviePage$(el).text().includes("Episode Links") ||
              moviePage$(el).text().includes("Single Episode"),
          )
          .parent()
          .attr("href");
      ...
      const downloadLinkWithToken = await axiosInstance.post(
        `${new URL(downloadLink!).origin}/red.php`,
        formData.toString(),
        {
          headers: {
            referer: this.url,
      ...
      let vcloudUrl = downloadPage$("a")
        ...
        .find(
          (a) =>
            downloadPage$(a).attr("href")?.includes("v-cloud.bio") ||
            downloadPage$(a).attr("href")?.includes("fast-dl.pro"),
        );

      if (type === "tv") {
        const episodesContainer = downloadPage$("h4 > span strong span").filter(
          (_, el) =>
            downloadPage$(el).text().includes(`-:Episodes: ${episode}:-`),
        );
        const episodeButtonContainer = episodesContainer
          .parent()
          .parent()
          .parent()
          .next();
